...
async fn main() -> Result<()> {
    ...
    for (k, v) in conf.clients {
        let encoded: Vec<u8> = ProxyFrame::Conf(k.clone(), v.local_port, v.remote_port)
            .try_into()
            .unwrap();
        tracing::debug!("send client config {:?}", &encoded);
        listener.write(&encoded).await?;
        if let Ok(conn) = TcpStream::connect(format!("127.0.0.1:{}", v.local_port)).await {
            l.proxy.insert(k, conn.clone());
            tracing::debug!("proxy stream locol addr is {}", conn.local_addr().unwrap());
        };
    }
    if let Some(conn) = l.proxy.get_mut(&"tcp1".to_string()) {
        loop {
            ...
            match listener.read(&mut buf).await {
                ...
                Ok(n) => {
                    ...
                    if let ProxyFrame::Body(name, addr, body) = decoded {
                        ...
                        // TODO: 回传数据
                        let encoded = bincode::serialize(&ProxyFrame::Body(
                            name,
                            addr,
                            buf[0..size].to_vec(),
                        ))
